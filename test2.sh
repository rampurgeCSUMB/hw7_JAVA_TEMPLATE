...
if [[ -f "./Main.java" ]]; then
  ...
  if [[ -d "test2/input" ]] && [[ -d "test2/output" ]]; then
    for input in test2/input/*.txt; do
      ((total++))
      base_name=$(basename "$input" .txt)
      output_file="test2/output/output${base_name:5}.txt"  # Change input to output
    ...
    echo "Error: Directories 'test2/input' or 'test2/output' not found."
    exit 1
  fi
  echo "-------------------------------"
